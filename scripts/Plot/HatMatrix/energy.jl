...
function plot_energy(name_model::Symbol)
    ##
    name_data = :CE
    normalization = :standard
    epoch = 150
    ##
    println("Energy: $(name_model), $(name_data), $(normalization), $(epoch)")
    ##
    bra_g = :g_identity
    bra_fn = :loss_energy
    supertitle = "Gradient Coherence Over Time"
    title = "($(name_model), $(string(name_data)[1:2]), Epoch $(epoch))"
    label_y = "Response time (Energy)"
    label_x = "Perturbation time (SMSE)"
    ##
    T = 16
    N = 3
    ...
    ##
    map_color = :amp
    lims_vals = maximum(abs.(vals))
    range_color = (0.0f0, lims_vals)
    ...
    ##
    path_save = plotsdir(
        "HatMatrix/$(name_data)/$(name_model)/$(normalization)/energy/heatmap_epoch_$(epoch).pdf",
    )
    wsave(path_save, fig)
    ##
    return fig
