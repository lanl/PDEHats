...
function plot_mass(name_model::Symbol)
    ##
    name_data = :CE
    normalization = :standard
    epoch = 150
    ##
    println("Mass: $(name_model), $(name_data), $(normalization), $(epoch)")
    ##
    bra_g = :g_identity
    bra_fn = :loss_mass
    supertitle = "Gradient Coherence Over Time"
    title = "($(name_model), $(string(name_data)[1:2]), Epoch $(epoch))"
    label_y = "Response time (Mass)"
    ...
    ##
    path_save = plotsdir(
        "HatMatrix/$(name_data)/$(name_model)/$(normalization)/mass/heatmap_epoch_$(epoch).pdf",
    )
    wsave(path_save, fig)
    ##
    return fig
